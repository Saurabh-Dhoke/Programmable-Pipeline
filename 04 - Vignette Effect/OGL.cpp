...
// for vigenite 
GLuint  darknessUniform = 0;
GLuint  offsetUniform = 0;

GLfloat offset_value = 0.0f;
GLfloat darkness_value = 0.0f;
//------------------------------
...
LRESULT CALLBACK WndProc(HWND   hwnd, UINT   iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR :
		...
		case 'A':
			offset_value = offset_value + 0.2f;
			break;
		...
		case 'S':
			darkness_value = darkness_value + 0.2f;
			break;
	...
	//-----------------------------------
	 ...
	 const GLchar* fragmentShaderSourceCode =
		 ...
		 "{" \
			...
			"vec4 inputColor   = texture(uTextureSampler, oTexCoord); " \
			"vec3 color        = inputColor.rgb; " \
			"vec2 uv           = oTexCoord; " \

			"vec2 coord = (uv - center) * vec2(uoffset);" \
			"color = mix(color, vec3(1.0f - udarkness), dot(coord, coord)); " \
	 ...
	 darknessUniform = glGetUniformLocation(shaderProgramObject, "uoffset");
	 offsetUniform = glGetUniformLocation(shaderProgramObject, "udarkness");
...
void display(void)
{
	...
	glUniform1f(offsetUniform, offset_value);
	glUniform1f(darknessUniform, darkness_value);
