...
/******************************************* 
	 Dissaturation Transition Effect  
	Date : 21.05.2024 
*******************************************/
...
// for dessaturation effect 
GLuint conversionFactorUniform = 0;
BOOL gbGrayScale = FALSE;
BOOL gbReset = FALSE;
...
int initialize(void)
{
	...
	//-----------------------------------
	 ...
	 const GLchar* fragmentShaderSourceCode =
		 ...
		 "uniform float u_ConversionFactor;" \
		 "out vec4 FragColor;" \
		 "void main(void)" \
		 "{" \
			"vec4 color ;" \
			"color = texture(uTextureSampler, oTexCoord);" \
			"float grayScaleFactor = (color.r * 0.3 + color.g * 0.59 + color.b * 0.11);" \
			"vec4 grayScaleColor = vec4(grayScaleFactor, grayScaleFactor, grayScaleFactor, 1.0) ;" \
			"FragColor = mix(color, grayScaleColor, u_ConversionFactor);" \
		 "}";
	 ...
	 conversionFactorUniform = glGetUniformLocation(shaderProgramObject, "u_ConversionFactor");
...
void display(void)
{
	static float conversionFactor = 0.0f;
	...
	if (gbGrayScale == TRUE)
	{
		...
		{
			conversionFactor = conversionFactor + 0.0005f;
		}
	}
	else
	{
