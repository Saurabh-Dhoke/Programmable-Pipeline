// Assignment 
/******************************************* 
	Per Fragment ...Per vertex Lighting  - Coloureed Albedo
	...
	Date : 08.04.2024 
	Base Code - Per Fragment and per vertexx albedo sphere program
...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode_PV =
		...
		"vec3 lightDirection     = normalize(vec3(ulightPosition - aPosition)); " \
		...
		"vec3 lightSpecular = uLightSpecular * uMaterialSpecular * pow(max(dot(reflectionVector,viewerVector), 0.0), uMaterialShinniness ); " \
		"oPhong_ADS_Light = ambiantLight + diffuseLight + lightSpecular; " \
		"} " \
		"else " \
		"{ " \
  ...
  // mvpMatrixUniform = glGetUniformLocation(shaderProgramObject_PV, "uMVPMatrix");
	...
	lightAmbiantUniform_PV  = glGetUniformLocation(shaderProgramObject_PV, "uLightAmbiant");
	...
	// mvpMatrixUniform = glGetUniformLocation(shaderProgramObject, "uMVPMatrix");
	 ...
	 lightAmbiantUniform_PF  = glGetUniformLocation(shaderProgramObject_PF, "uLightAmbiant");
