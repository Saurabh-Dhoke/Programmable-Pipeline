...
GLubyte checkImage[CHECK_IMAGE_HEIGHT][CHECK_IMAGE_WIDTH][4];  // 3D Array y 
...
int initialize(void)
{
	...
	void uninitialize();
	void resize(int, int);
	void loadGLTexture(void);    // for texture
	...
	glClearColor(0.75f, 0.75f, 0.75f, 1.0f); // blue color de as sangital ct ith color ith nhi dila jat
...
void loadGLTexture(void)
{
	...
	// code
	// check image array gets filled in this function
	makeCheckImage();
	...
	// unpacked image				
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);// monochrome blck and white mhanun 4 chya jagi 1 takal
...
void display(void)
{
	...
	// Transformation 
	modelViewProjectionMatrix = PerspectiveProjectionMatrix * modelViewMatrix; // order of multiplication is VIMP

	// Push above MVP into vertex shaders mvpuniform 
	glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, modelViewProjectionMatrix);

	square_position[0] = 2.41421f;
	square_position[1] = 1.0f;
	square_position[2] = -1.41421f;

	square_position[3] = 1.0f;
	square_position[4] = 1.0f;
	square_position[5] = 0.0f;

	square_position[6] =  1.0f;
	square_position[7] = -1.0f;
	square_position[8] =  0.0f;

	square_position[9] =   2.41421f;
	square_position[10] = -1.0f;
	square_position[11] = -1.41421f;
